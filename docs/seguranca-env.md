...
Variaveis com prefixo `VITE_` sao expostas no bundle do frontend. Use esse prefixo apenas para valores publicos, como URL publica do Supabase e chave anon/publishable.

Variaveis privadas como `DATABASE_URL`, `JWT_SECRET` e `SUPABASE_SERVICE_ROLE_KEY` devem existir somente em backend/API ou no provedor de deploy. Elas nao devem ser lidas por codigo Vite/React nem enviadas ao navegador.
